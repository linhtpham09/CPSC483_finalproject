...
for epoch in range(args.epoch):
    ...
    *********************************************************
    ...
    for idx in range(n_batch//4):
        btime= time()
        print(f'index: {idx}, {idx/(n_batch//4)}')
    ...
    *********************************************************
    ...
    for idx in range(n_A_batch//4):
        print(f'Iteration {idx}/{n_A_batch//4}')  # Displays the current iteration and total iterations
        btime = time()
    ...
    *********************************************************
    ...
    # Skip testing if no users are available
    if len(users_to_test) == 0:
        print("No users to test. Skipping.")
        continue
    ...
    # Append metrics if valid
    if 'recall' in ret and len(ret['recall']) > 0:
        rec_loger.append(ret['recall'])
    else:
        print(f"Invalid recall data: {ret}")

    if 'precision' in ret and len(ret['precision']) > 0:
        pre_loger.append(ret['precision'])
    else:
        print(f"Invalid precision data: {ret}")

    if 'ndcg' in ret and len(ret['ndcg']) > 0:
        ndcg_loger.append(ret['ndcg'])
    else:
        print(f"Invalid NDCG data: {ret}")

    if 'hit_ratio' in ret and len(ret['hit_ratio']) > 0:
        hit_loger.append(ret['hit_ratio'])
    else:
        print(f"Invalid hit ratio data: {ret}")
...
if len(rec_loger) == 0:
    print("No recall values logged. Exiting.")
    sys.exit(1)
    
recs = np.array(rec_loger)
print(f"Shape of recs: {recs.shape}, content: {recs}")
...
# Find best recall and corresponding index
best_rec_0 = max(recs[:, 0]) if recs.ndim > 1 else max(recs)
idx = list(recs[:, 0]).index(best_rec_0) if recs.ndim > 1 else recs.tolist().index(best_rec_0)
