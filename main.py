...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.save_flag == 1:
        if args.model_type in ['bprmf', 'cke', 'fm', 'cfkg']:
            weights_save_path = '%s/weights/%s/l%s_r%s' % (
                args.hop + "hop", layer, str(args.lr), '-'.join([str(r) for r in eval(args.regs)]))
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        save_path = '%s/%s_report.result' % (args.hop + "hop",args.hop + "hop")
        ensureDir(save_path)
        f = open(save_path, 'w')
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        if args.model_type in ['kgat']:
            ...
            if args.use_kge is True:
                ...
                for idx in range(n_A_batch//4):
                    if idx % 100 == 0: 
                        print(f'Iteration {idx}/{n_A_batch//4}')
                    btime = time()
        ...
        *********************************************************
        ...
        loss = loss.item() if isinstance(loss, np.ndarray) and loss.size == 1 else loss
        base_loss = base_loss.item() if isinstance(base_loss, np.ndarray) and base_loss.size == 1 else base_loss
        kge_loss = kge_loss.item() if isinstance(kge_loss, np.ndarray) and kge_loss.size == 1 else kge_loss
        reg_loss = reg_loss.item() if isinstance(reg_loss, np.ndarray) and reg_loss.size == 1 else reg_loss
    ...
    save_path = '%s/%s_output.result' % (args.hop + "hop",args.hop + "hop")
    ensureDir(save_path)
